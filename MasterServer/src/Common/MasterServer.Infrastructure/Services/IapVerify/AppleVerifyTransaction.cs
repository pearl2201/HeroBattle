...
namespace Iap.Verify
{
    public class AppleVerifyTransaction : Apple
    {
        ...
        private ValidationIapResult ValidateTransaction(Receipt receipt, StoreKitTransactionInfo transactionInfo, ILogger log)
        {
            var result = default(ValidationIapResult);
            ...
            try
            {
                ...
                {
                    result = new ValidationIapResult(false, $"transaction id '{receipt.TransactionId}' does not match '{transactionInfo.TransactionId}'");
                }
                else if (!string.Equals(transactionInfo.BundleId, receipt.BundleId, StringComparison.Ordinal))
                {
                    result = new ValidationIapResult(false, $"bundle id '{receipt.BundleId}' does not match '{transactionInfo.BundleId}'");
                }
                else
                {
