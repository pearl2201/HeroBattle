...
namespace LiteEntitySystem.FixedMath
{
    public class Matrix4x4f : IEquatable<Matrix4x4f>
    {
        ...
        public static Matrix4x4f CreateBillboard(Vector3f objectPosition, Vector3f cameraPosition, Vector3f cameraUpVector, Vector3f cameraForwardVector)
        {
            ...
            {
                zaxis = Vector3f.Multiply(zaxis, 1.0f / (float)Math.Sqrt(norm));
            }
        ...
        public static Matrix4x4f CreateConstrainedBillboard(Vector3f objectPosition, Vector3f cameraPosition, Vector3f rotateAxis, Vector3f cameraForwardVector, Vector3f objectForwardVector)
        {
            ...
            {
                faceDir = Vector3f.Multiply(faceDir, (1.0f / (float)Math.Sqrt(norm)));
            }

            Vector3f yaxis = rotateAxis;
            Vector3f xaxis;
            Vector3f zaxis;

            // Treat the case when angle between faceDir and rotateAxis is too close to 0.
            float dot = Vector3f.Dot(rotateAxis, faceDir);
            ...
            if (Math.Abs(dot) > minAngle)
            {
                ...
                dot = Vector3f.Dot(rotateAxis, zaxis);

                if (Math.Abs(dot) > minAngle)
                {
                    zaxis = (Math.Abs(rotateAxis.Z) > minAngle) ? new Vector3f(1, 0, 0) : new Vector3f(0, 0, -1);
                }

                xaxis = Vector3f.Normalize(Vector3f.Cross(rotateAxis, zaxis));
                zaxis = Vector3f.Normalize(Vector3f.Cross(xaxis, rotateAxis));
            }
            else
            {
                xaxis = Vector3f.Normalize(Vector3f.Cross(rotateAxis, faceDir));
                zaxis = Vector3f.Normalize(Vector3f.Cross(xaxis, yaxis));
            }
        ...
        /// <returns>The scaling matrix.</returns>
        public static Matrix4x4f CreateScale(Vector3f scales)
        {
            Matrix4x4f result = default(Matrix4x4f);
        ...
        /// <param name="centerPoint">The center point.</param>
        /// <returns>The scaling matrix.</returns>
        public static Matrix4x4f CreateScale(Vector3f scales, Vector3f centerPoint)
        {
            Matrix4x4f result = default(Matrix4x4f);
        ...
        public static bool Invert(Matrix4x4f matrix, out Matrix4x4f result)
        {
            //                                       -1
            ...
            {
                result = new Matrix4x4f(float.NaN, float.NaN, float.NaN, float.NaN,
        ...
        [System.Security.SecuritySafeCritical]
        public static bool Decompose(Matrix4x4f matrix, out Vector3f scale, out Quaternion rotation, out Vector3f translation)
        {
            bool result = true;
            ...
            unsafe
            {
                fixed (Vector3f* scaleBase = &scale)
                {
                    ...
                    Matrix4x4f matTemp = Matrix4x4f.Identity;
                    ...
                    *(pVectorBasis[0]) = new Vector3f(matrix.M11, matrix.M12, matrix.M13);
                    *(pVectorBasis[1]) = new Vector3f(matrix.M21, matrix.M22, matrix.M23);
                    *(pVectorBasis[2]) = new Vector3f(matrix.M31, matrix.M32, matrix.M33);
        ...
        public static bool operator ==(Matrix4x4f value1, Matrix4x4f value2)
        {
