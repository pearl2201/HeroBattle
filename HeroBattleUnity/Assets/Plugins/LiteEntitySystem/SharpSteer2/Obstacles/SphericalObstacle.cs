...
namespace SharpSteer2.Obstacles
{
	...
	public class SphericalObstacle : IObstacle
	{
	    ...
	    public SphericalObstacle()
			: this(1, Vector3f.Zero)
		{
		}

        public SphericalObstacle(float r, Vector3f c)
		{
			Radius = r;
			Center = c;
		}
        ...
        /// <returns></returns>
        public Vector3f SteerToAvoid(IVehicle v, float minTimeToCollision)
        {
            ...
            if (capLength <= 0.05)
            {
                var distance = Vector3f.Distance(Center, v.Position);
            ...
            Vector3f avoidance = Vector3fHelpers.PerpendicularComponent(v.Position - Center, v.Forward);
            ...
            if (avoidance == Vector3f.Zero)
                avoidance = -v.Forward + v.Side * 0.01f * RandomHelpers.Random();

            avoidance = Vector3f.Normalize(avoidance);
        ...
        public float? NextIntersection(IVehicle vehicle)
        {
            ...
            Vector3f lc = vehicle.LocalizePosition(Center);
