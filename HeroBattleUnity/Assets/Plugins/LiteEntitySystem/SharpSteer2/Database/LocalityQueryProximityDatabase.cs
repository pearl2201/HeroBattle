...
namespace SharpSteer2.Database
{
	...
	public class LocalityQueryProximityDatabase<T> : IProximityDatabase<T> where T : class
	{
		...
		private sealed class TokenType : ITokenForProximityDatabase<T>
		{
            ...
            public void UpdateForNewPosition(Vector3f p)
			{
				_lq.UpdateForNewLocation(_proxy, p);
			}

			// find all neighbors within the given sphere (as center and radius)
            public void FindNeighbors(Vector3f center, float radius, List<T> results)
			{
		...
		// constructor
        public LocalityQueryProximityDatabase(Vector3f center, Vector3f dimensions, Vector3f divisions)
		{
			Vector3f halfsize = dimensions * 0.5f;
			Vector3f origin = center - halfsize;
