...
namespace SharpSteer2.Database
{
	...
	class LocalityQueryDatabase
	{
		 ...
		 * contents.
		 */
		public LocalityQueryDatabase(Vector3f origin, Vector3f size, int divx, int divy, int divz)
		{
			_origin = origin;
			_size = size;
			_divX = divx;
		...
		public void UpdateForNewLocation(ClientProxy obj, Vector3f position)
		{
	    ...
	    private /*lqClientProxy*/int BinForLocation(Vector3f position)
		{
		...
		/* Apply an application-specific function to all objects in a certain
		   ...
		   bins of interest. */
		public void MapOverAllObjectsInLocality(Vector3f center, float radius, LQCallBackFunction func, Object clientQueryState)
		{
			int partlyOut = 0;
	    ...
	    private static void TraverseBinClientObjectList(ClientProxy co, float radiusSquared, LQCallBackFunction func, Object state, Vector3f position)
		{
			while (co != null)
			{
				// compute distance (squared) from this client obj to given
				// locality sphere's centerpoint
				Vector3f d = position - co.Position;
	    ...
	    private void MapOverAllObjectsInLocalityClipped(Vector3f center, float radius,
        ...
        /// <param name="func"></param>
        /// <param name="clientQueryState"></param>
	    private void MapOverAllOutsideObjects(Vector3f center, float radius, LQCallBackFunction func, Object clientQueryState)
		{
