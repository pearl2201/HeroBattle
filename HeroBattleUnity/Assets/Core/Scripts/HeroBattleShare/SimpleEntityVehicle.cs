...
{
    public abstract class SimpleEntityVehicle : BaseEntityVehicle
    {
        Vector3f _lastForward;
        Vector3f _lastPosition;
        float _smoothedCurvature;
        // The acceleration is smoothed
        Vector3f _acceleration;

        protected SimpleEntityVehicle(EntityParams entityParams) : base(entityParams)
        {
            Reset();
        }
        // reset vehicle state
        ...
        public override Vector3f Velocity
        {
        ...
        public void ApplySteeringForce(Vector3f force, float elapsedTime)
        {
            Vector3f adjustedForce = AdjustRawSteeringForce(force, elapsedTime);

            // enforce limit on magnitude of steering force
            Vector3f clippedForce = adjustedForce.TruncateLength(MaxForce);

            // compute acceleration and velocity
            Vector3f newAcceleration = (clippedForce / Mass);
        ...
        protected virtual void RegenerateLocalSpace(Vector3f newVelocity, float elapsedTime)
        {
        ...
        protected void RegenerateLocalSpaceForBanking(Vector3f newVelocity, float elapsedTime)
        {
            // the length of this global-upward-pointing vector controls the vehicle's
            // tendency to right itself as it is rolled over from turning acceleration
            Vector3f globalUp = new Vector3f(0, 0.2f, 0);

            // acceleration points toward the center of local path curvature, the
            // length determines how much the vehicle will roll while turning
            Vector3f accelUp = _acceleration * 0.05f;

            // combined banking, sum of UP due to turning and global UP
            Vector3f bankUp = accelUp + globalUp;

            // blend bankUp into vehicle's UP basis vector
            float smoothRate = elapsedTime * 3;
            Vector3f tempUp = Up;
            Utilities.BlendIntoAccumulator(smoothRate, bankUp, ref tempUp);
            Up = Vector3f.Normalize(tempUp);
        ...
        /// <returns></returns>
        protected virtual Vector3f AdjustRawSteeringForce(Vector3f force, float deltaTime)
        {
            float maxAdjustedSpeed = 0.2f * MaxSpeed;

            if ((Speed > maxAdjustedSpeed) || (force == Vector3f.Zero))
                return force;
        ...
        /// <returns></returns>
        public override Vector3f PredictFuturePosition(float predictionTime)
        {
        ...
        {
            _lastForward = Vector3f.Zero;
            _lastPosition = Vector3f.Zero;
            _smoothedCurvature = value;
            Curvature = value;
        }

        public override Vector3f Acceleration
        {
        ...
        {
            ResetAcceleration(Vector3f.Zero);
        }

        private void ResetAcceleration(Vector3f value)
        {
            _acceleration = value;
        }

        Vector3f _smoothedPosition;

        public Vector3f SmoothedPosition
        {
        ...
        {
            ResetSmoothedPosition(Vector3f.Zero);
        }

        protected void ResetSmoothedPosition(Vector3f value)
        {
        ...
        {
            Up = Vector3f.UnitY;
            Forward = Vector3fHelpers.RandomUnitVectorOnXZPlane();
        ...
        void MeasurePathCurvature(float elapsedTime)
        {
            if (elapsedTime > 0)
            {
                Vector3f dP = _lastPosition - Position;
                Vector3f dF = (_lastForward - Forward) / dP.Length();
                Vector3f lateral = Vector3fHelpers.PerpendicularComponent(dF, Forward);
                float sign = (Vector3f.Dot(lateral, Side) < 0) ? 1.0f : -1.0f;
